...
CREATE TABLE employee_profiles (
    ...
    profile NVARCHAR(MAX)  -- JSON data column
);


INSERT INTO employee_profiles (employee_id, profile)
VALUES
(100, '{
    "skills": ["Leadership", "Strategic Management", "Decision Making"],
    "certifications": ["Executive Leadership Certificate"],
    "languages": ["English"],
    "experience_years": 20
}'),
(103, '{
    "skills": ["SQL Server", "Power BI", "ETL"],
    "certifications": ["Microsoft Certified: Data Analyst Associate"],
    "languages": ["English"],
    "experience_years": 8
}'),
(108, '{
    "skills": ["Accounting", "Excel", "Finance Reporting"],
...
## In this project, I designed and analyzed an HR database in SQL Server to gain insights into employee performance,
department structure, and compensation.
I used JSON columns to store flexible data (skills and dependents), CASE statements for conditional analysis,
and window functions to identify top performers and salary gaps across departments.
Question 1:

Find each employee’s name, experience years and a seniority category based on that experience.
that experience.

If experience ≥ 10 years → “Senior”

Between 5 and 9 → “Mid-Level”
...
SELECT 
    ...
    JSON_VALUE(p.profile, '$.experience_years') AS ExperienceYears,
    CASE 
        WHEN CAST(JSON_VALUE(p.profile, '$.experience_years') AS INT) >= 10 THEN 'Senior'
        WHEN CAST(JSON_VALUE(p.profile, '$.experience_years') AS INT) BETWEEN 5 AND 9 THEN 'Mid-Level'
        ELSE 'Junior'
...
Second question
Show a list of skills for each employee and rank employees based on the number of skills they have
WITH SkillsCount AS (
    SELECT 
        e.employee_id,
        e.first_name + ' ' + e.last_name AS EmployeeName,
        COUNT(value) AS SkillCount
    FROM employee_profiles p
    JOIN employees e ON e.employee_id = p.employee_id
    CROSS APPLY OPENJSON(p.profile, '$.skills')
    GROUP BY e.employee_id, e.first_name, e.last_name
)
SELECT 
    EmployeeName,
    SkillCount,
    RANK() OVER(ORDER BY SkillCount DESC) AS SkillRank
FROM SkillsCount;


Question3
Find employees who speak more than one language and show:
...
salary rank within their department Also, classify them by income level using CASE:

Salary ≥ 15000 → “High”

7000–14999 → “Medium”Below 7000 → “Low”
...
;WITH LanguageCount AS (
    SELECT  
        ...
        e.first_name + ' ' + e.last_name AS EmployeeName, 
        e.salary,
        COUNT(value) AS LanguageCount
    FROM employee_profiles p
    JOIN employees e ON e.employee_id = p.employee_id
    CROSS APPLY OPENJSON(p.profile, '$.languages')
    GROUP BY 
...
SELECT 
    ...
    CASE 
        ...
        WHEN salary BETWEEN 7000 AND 14999 THEN 'Medium'
        ELSE 'Low'
...
Question:
For each manager, calculate total employees managed,
  average salary of their team, and

label them as:  “Large Team” if managing ≥ 5 people,  “Small Team” if managing < 5 people.

WITH ManagerStats AS (
    SELECT 
        e.manager_id, COUNT(e.employee_id) AS TeamSize, AVG(e.salary) AS AvgTeamSalary
    FROM employees e
    WHERE e.manager_id IS NOT NULL
    GROUP BY e.manager_id)
SELECT 
    m.first_name + ' ' + m.last_name AS ManagerName,
    s.TeamSize, ROUND(s.AvgTeamSalary, 2) AS AvgTeamSalary,
    CASE 
        WHEN s.TeamSize >= 5 THEN 'Large Team'
        ELSE 'Small Team'
    END AS TeamCategory
FROM ManagerStats s
JOIN employees m ON s.manager_id = m.employee_id
ORDER BY s.TeamSize DESC;


question 6

find the salary difference between the highest and lowest paid employee.
Show department name, highest salary, lowest salary, and the gap.

SELECT 
    d.department_name,
    MAX(e.salary) AS HighestSalary,
    MIN(e.salary) AS LowestSalary,
    MAX(e.salary) - MIN(e.salary) AS SalaryGap
FROM employees e
JOIN departments d ON e.department_id = d.department_id
